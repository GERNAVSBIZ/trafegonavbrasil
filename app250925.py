...
def parse_data_file(file_content, icao_code):
    ...
    for line in lines:
        ...
        try:
            ...
            if overflight_match:
                waypoints.extend([overflight_match.group(1), overflight_match.group(3)])
...
# MODIFICADO: Rota para salvar uma análise completa com nome personalizado
...
def save_records():
    ...
    try:
        ...
        first_data_date = all_records_for_this_analysis[0].get('timestamp')
...
# MODIFICADO: Rota para obter uploads agora retorna o nome da análise
    ...
    try:
        ...
        for doc in query.stream():
            ...
            results.append({
                ...
                'analysisName': doc_data.get('analysisName', f"Análise de {doc_data['createdAt'].strftime('%d/%m/%Y')}")
            })
...
# As rotas restantes (get_records, delete_upload, get_aggregated_data) permanecem sem alterações
